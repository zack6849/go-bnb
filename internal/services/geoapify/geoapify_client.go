...
func (c *Client) Geocode(query string, level AddressLevel) {
...
func (c *Client) ReverseGeocode(point *geo.Point, resultType ResultSpecificityLevel, ctx context.Context) (ReverseGeocodingResult, error) {
	zero := ReverseGeocodingResult{}
	target := ReverseGeocodingResponse{}
	params := c.getDefaultRequestParams()
	params.Set("type", string(resultType))
	params.Set("format", "json")
